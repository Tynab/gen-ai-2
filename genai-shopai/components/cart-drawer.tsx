...
// Khác với ProductCard, drawer này KHÔNG thuần props: form checkout (tên, sđt,
// địa chỉ) tự giữ state riêng và gọi thẳng createOrder() ở đây, không đi qua
// callback nào từ app/page.tsx — chỉ list giỏ hàng/tổng tiền là điều khiển bằng props.
...
export function CartDrawer({
  ...
  async function handleCheckout() {
    // Validation duy nhất ở đây là "còn item trong giỏ" — số lượng tồn kho đã được
    // giới hạn từ trước bởi Product.stock khi cộng/trừ số lượng ở app/page.tsx;
    // việc kiểm tra tồn kho lần cuối và trừ kho thật sự nằm ở backend (create_order).
            ...
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.product.id} className="flex gap-3 border-b border-slate-100 pb-4">
                  {/* Thumbnail cố định 80x80 (h-20 w-20) nên khai width/height tĩnh thay vì fill —
                      kích thước không đổi theo viewport nên cũng không cần prop sizes. */}
                  <Image src={item.product.image_url} alt={item.product.name} width={80} height={80} className="h-20 w-20 rounded-md object-cover" />
