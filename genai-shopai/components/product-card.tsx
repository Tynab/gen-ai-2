...
// Card sản phẩm thuần props/callback — không giữ state riêng, mọi tương tác (mở
// chi tiết, thêm giỏ hàng) đều gọi ngược lên app/page.tsx qua onOpen/onAddToCart.
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-brand-orange/40 hover:shadow-md">
      <button className="relative block aspect-square bg-slate-100 text-left" onClick={() => onOpen(product)}>
        {/* next/image với fill trong khung aspect-square; sizes bám theo grid sản phẩm ở app/page.tsx
            (2 cột mobile → 50vw, 3 cột md → 33vw, 5 cột xl → 20vw) để trình duyệt không tải ảnh quá khổ. */}
        <Image src={product.image_url} alt={product.name} fill sizes="(max-width: 768px) 50vw, (max-width: 1280px) 33vw, 20vw" className="object-cover" />
        {discount > 0 ? (
